...
import bigappcompany.com.myoffers.adapter.CategoryTabAdapter;
 ...
 * @created on 05 Jul 2017 at 12:49 PM
 */

/*changes in new apk(Stop refreshing fragment on screen visibility)
*  171 - 188
*
* */
...
public class HomeFragment extends Fragment {
	...
	private CategoryTabAdapter cattab_adapter;
	...
	private Set<CatDataModel> hs = new HashSet<>();
	...
	private ArrayList<LocationModel> locdata = new ArrayList<>();
	private View v;
	...
	public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
		...
		cattab_rec.setNestedScrollingEnabled(false);
			...
			public void onTaskCompleted(String response) {
				...
				if (response != null && response.length() > 0) {
					...
					if (categoryModel.getStatus().equalsIgnoreCase("true")) {
						...
						storedArrayList.clear();
						
						modelArrayList = categoryModel.getData();
						storedArrayList = categoryModel.getData();
	...
	private void getdistance(ArrayList<CatDataModel> modlArrayList) {
		...
		cattab_adapter = new CategoryTabAdapter(getActivity(), modelArrayList, locdata, HomeFragment.this);
	...
	public void senrecentdpos(int position) {
		startActivity(new Intent(getActivity(), Description.class).putExtra("name", descProdModels.get(position)
		    .getShop_name()).putExtra("address", descProdModels.get(position).getAddress_line_1()).putExtra("lat",
		    descProdModels.get(position).getLatitude()).putExtra("lon", descProdModels.get(position).getLongitude())
		    .putExtra("local", "yes"));
	}
